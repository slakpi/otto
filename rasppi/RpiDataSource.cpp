...
/**
 * Fixed magnetic deviation corrections measured after applying dynamic hard-
 * and soft-iron offset / scale corrections.
 */
...
static void quaternionToYawPitchRoll(float q[4], float e[3])
{
  ...
  /**
   * Yaw is in the [-180, 180) range. We need to correct this to [0, 360) as
   * well as add an additional 90 degrees for the board orientation.
   */
...
void* RpiDataSource::threadProc(void *_ptr)
{
  ...
  /**
   * Setup the Madgwick AHRS filter with a beta 20 times higher than the
   * default. This will let it settle on values faster. Presumably, there is
   * some trade-off here.
   */
  beta = 2.0f;
  ...
  while (true)
  {
    ...
    pthread_mutex_lock(&rds->sampleLock);
       ...
       * guaranteed to get all parameters during each loop iteration. So, leave
       * the previous values.
       */
      if (magOk)
