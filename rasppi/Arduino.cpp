...
float Arduino::getServoPos() const
{
  ...
  /**
   * See setServoPos() for the scaling function.  This is just the reverse of
   * that function.
   */
...
void Arduino::setServoPos(float _Prel)
{
  ...
  /**
   * The servo operates in the range [0, 180] degrees where 90 degrees is
   * center. We're going to limit the range to [60, 120] degrees or center
   * -/+ 30 degrees.
   *
   * Clamp the relative position value to the range [-1, 1], then scale the
   * value to the range [-30, 30] and shift it up to the range [60, 120].
   *
   *                      120 - 60
   * Pdeg = (Prel - -1) * -------- - 30 + 90
   *                       1 - -1
